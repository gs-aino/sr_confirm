...
datatable_cols = ["sr_no", "sr_cate", "sr_text", # "act_text",
        "prd_cd", "prd_nm", "ord_cd", "ord_date", "ord_status"]
...
def upload():
    ...
    # df_tags["tags"] = "1: "+df_tags["tag_first_name"]+" / 2: "+df_tags["tag_second_name"]+" / 3: "+df_tags["tag_third_name"]
    #
    # print('''df_datatable''')
    # df_datatable = df_temp.loc[df_temp['sr_cate_b'].isin(['교환/반품/AS관련', '상품품질관련', '상품관련']), datatable_cols].reset_index(drop=True)
    # df_datatable = df_datatable.merge(df_tags[['sr_no', 'tags']], on=['sr_no'])
